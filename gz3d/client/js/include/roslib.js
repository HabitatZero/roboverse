 ...
 *   * actionName - the action message name, like
...
ROSLIB.ActionClient = function(options) {
  ...
  // If timeout specified, emit a 'timeout' event if the action server does not
  // respond
  if (this.timeout) {
    setTimeout(function() {
      if (!receivedStatus) {
...
ROSLIB.Goal = function(options) {
  ...
  this.on("status", function(status) { that.status = status; });
  ...
  this.on("feedback", function(feedback) { that.feedback = feedback; });
 ...
 *
 * @constructor
 * @param values - object matching the fields defined in the .msg definition
 *     file
 */
ROSLIB.Message = function(values) {
  ...
  Object.keys(values).forEach(function(name) { that[name] = values[name]; });
};
 ...
 * @constructor
 * @param options - possible keys include:
 *   * url (optional) - the WebSocket URL for rosbridge (can be specified later
 * with `connect`)
 */
...
ROSLIB.Ros.prototype.connect = function(url) {
   ...
   */
  function onOpen(event) { that.emit("connection", event); }
   ...
   * @param event - the argument to emit with the event.
   */
  function onClose(event) { that.emit("close", event); }
   ...
   * @param event - the argument to emit with the event.
   */
  function onError(event) { that.emit("error", event); }
  ...
  function decompressPng(data, callback) {
    // Uncompresses the data before sending it through (use image/canvas to do
    // so).
    ...
    image.onload = function() {
      ...
      for (let i = 0; i < imageData.length; i += 4) {
        // RGB
        jsonData += String.fromCharCode(imageData[i], imageData[i + 1],
                                        imageData[i + 2]);
      }
  ...
  function onMessage(message) {
    ...
    if (data.op === "png") {
      decompressPng(
          data,
          function(decompressedData) { handleMessage(decompressedData); });
    } else {
      handleMessage(data);
    }
  }
 ...
 */
ROSLIB.Ros.prototype.authenticate = function(mac, client, dest, rand, t, level,
                                             end) {
  // create the request
  const auth = {
    op : "auth",
...
ROSLIB.Ros.prototype.callOnConnection = function(message) {
  ...
  if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
    that.once("connection", function() { that.socket.send(messageJson); });
  } else {
...
ROSLIB.Ros.prototype.getTopics = function(callback) {
  ...
  topicsClient.callService(request,
                           function(result) { callback(result.topics); });
};
...
ROSLIB.Ros.prototype.getServices = function(callback) {
  ...
  servicesClient.callService(request,
                             function(result) { callback(result.services); });
};
...
ROSLIB.Ros.prototype.getParams = function(callback) {
  ...
  paramsClient.callService(request,
                           function(result) { callback(result.names); });
};
...
ROSLIB.Service.prototype.callService = function(request, callback) {
  ...
  const requestValues = [];
  Object.keys(request).forEach(function(
      name) { requestValues.push(request[name]); });
 ...
 *
 * @constructor
 * @param values - object matching the fields defined in the .srv definition
 *     file
 */
...
ROSLIB.Topic.prototype.subscribe = function(callback) {
  ...
  this.on("message", function(message) { callback(message); });
 ...
 */
ROSLIB.Pose.prototype.clone = function() { return new ROSLIB.Pose(this); };
 ...
 */
ROSLIB.Quaternion.prototype.clone =
    function() { return new ROSLIB.Quaternion(this); };
 ...
 */
ROSLIB.Transform.prototype.clone =
    function() { return new ROSLIB.Transform(this); };
 ...
 */
ROSLIB.Vector3.prototype.clone =
    function() { return new ROSLIB.Vector3(this); };
...
ROSLIB.TFClient.prototype.subscribe = function(frameID, callback) {
  ...
  // if there is no callback registered for the given frame, create emtpy
  // callback list
...
ROSLIB.UrdfModel = function(options) {
  ...
  const initXml = function(xml) {
    // Get the robot tag
    const robotXml = xml.evaluate("//robot", xml, null,
  ...
  const initXml = function(xml) {
    ...
    if (origins.length === 0) {
      ...
      if (rpy) {
        ...
        const the = pitch / 2.0;
        const psi = yaw / 2.0;
        const x = Math.sin(phi) * Math.cos(the) * Math.cos(psi) -
                  Math.cos(phi) * Math.sin(the) * Math.sin(psi);
        const y = Math.cos(phi) * Math.sin(the) * Math.cos(psi) +
                  Math.sin(phi) * Math.cos(the) * Math.sin(psi);
        const z = Math.cos(phi) * Math.cos(the) * Math.sin(psi) -
                  Math.sin(phi) * Math.sin(the) * Math.cos(psi);
        const w = Math.cos(phi) * Math.cos(the) * Math.cos(psi) +
