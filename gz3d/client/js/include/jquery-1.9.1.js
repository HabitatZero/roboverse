...
const fcamelCase = function(all, letter) { return letter.toUpperCase(); };
...
jQuery.fn = jQuery.prototype = {
  ...
  slice :
      function() { return this.pushStack(core_slice.apply(this, arguments)); },
...
jQuery.extend({
  ...
  isArray :
      Array.isArray || function(obj) { return jQuery.type(obj) === "array"; },

  isWindow : function(obj) { return obj != null && obj == obj.window; },

  isNumeric : function(
      obj) { return !isNaN(parseFloat(obj)) && isFinite(obj); },
...
jQuery.ready.promise = function(obj) {
  if (!readyList) {
    ...
    // has already occurred. we once tried to use readyState "interactive" here,
    // but it caused issues like the one discovered by ChrisS here:
...
jQuery.extend({
  Deferred : function(func) {
    ...
    var promise = {
      ...
      promise : function(
          obj) { return obj != null ? jQuery.extend(obj, promise) : promise; },
    };
    var deferred = {};
...
jQuery.support = (function() {
  ...
  support = {
    ...
    // jQuery.support.boxModel DEPRECATED in 1.8 since we don't support Quirks
    // Mode
  ...
  jQuery(function() {
    ...
    // Use window.getComputedStyle because jsdom on node.js will break without
    // it.
...
function internalData(elem, name, data, pvt /* Internal Use Only */) {
  ...
  // gets shallow copied over onto the existing cache
  ...
  // data cache in order to avoid key collisions between internal data and
  // user-defined data.
  if (!pvt) {
    if (!thisCache.data) {
      thisCache.data = {};
...
jQuery.extend({
  ...
  _data : function(elem, name,
                   data) { return internalData(elem, name, data, true); },
...
jQuery.fn.extend({
  ...
  removeData : function(
      key) { return this.each(function() { jQuery.removeData(this, key); }); },
});
...
jQuery.extend({
  ...
  dequeue : function(elem, type) {
    ...
    const next = function() { jQuery.dequeue(elem, type); };
...
jQuery.fn.extend({
  ...
  removeProp : function(name) {
    ...
    return this.each(function() {
      // try/catch handles cases where IE balks (such as removing a property on
      // window)
      try {
        this[name] = undefined;
        delete this[name];
  ...
  addClass : function(value) {
    ...
    if (jQuery.isFunction(value)) {
      return this.each(function(
          j) { jQuery(this).addClass(value.call(this, j, this.className)); });
    }
    ...
    type : {
      set : function(elem, value) {
        if (!jQuery.support.radioValue && value === "radio" &&
          ...
          // in IE6-9 Reset value to default in case type is set after value
          // during creation
...
boolHook = {
  get : function(elem, name) {
    ...
    const detail = typeof prop === "boolean"
                       ? getSetInput && getSetAttribute
                             ? attr != null
                             : // oldIE fabricates an empty string for missing
                               // boolean attributes
                             // and conflates checked/selected into attroperties
                             ruseDefault.test(name)
                                 ? elem[jQuery.camelCase("default-" + name)]
                                 : !!attr
                       : // fetch an attribute node for properties not
...
if (!getSetAttribute) {
  ...
  jQuery.attrHooks.contenteditable = {
    ...
    set : function(
        elem, value,
        name) { nodeHook.set(elem, value === "" ? false : value, name); },
  };
...
if (!jQuery.support.style) {
  jQuery.attrHooks.style = {
    get : function(elem) {
      ...
      // .toLowerCase() .cssText, that would destroy case senstitivity in URL's,
      // like in "background"
...
if (!jQuery.support.checkOn) {
  jQuery.each([ "radio", "checkbox" ], function() {
    jQuery.valHooks[this] = {
      get : function(elem) {
        // Handle the case where in Webkit "" is returned instead of "on" if a
...
jQuery.event = {
  ...
  trigger : function(event, data, elem, onlyHandlers) {
    ...
    let namespaces =
        core_hasOwn.call(event, "namespace") ? event.namespace.split(".") : [];
    ...
    if (!onlyHandlers && !event.isDefaultPrevented()) {
      if ((!special._default ||
        ...
        // event. Can't use an .isFunction() check here because IE6/7 fails that
        // test. Don't do default actions on window, that's where global
        // variables be (#6170)
  ...
  jQuery.event.special.change = {
    setup : function() {
      if (rformElems.test(this.nodeName)) {
        ...
        // click after a propertychange. Eat the blur-change in
        // special.change.handle. This still fires onchange a second time for
    ...
    handle : function(event) {
      ...
      // Swallow native change events from checkbox/radio, we already triggered
      // them above
...
jQuery.fn.extend({
  ...
  one : function(types, selector, data,
                 fn) { return this.on(types, selector, data, fn, 1); },
...
const operators = "([*^$|!~]?=)";
const attributes = "\\[" + whitespace + "*(" + characterEncoding + ")" +
...
const rcombinators = new RegExp("^" + whitespace + "*([\\x20\\t\\r\\n\\f>+~])" +
                                whitespace + "*");
 ...
 *on itself with property name the (space-suffixed) string and (if the cache is
...
setDocument = Sizzle.setDocument = function(node) {
  ...
  // Check if getElementsByName privileges form controls or returns elements by
  // ID
  ...
  Expr.attrHandle =
      assert(function(div) {
        ...
        return (div.firstChild &&
          ...
          ? {}
          : {
              href : function(elem) { return elem.getAttribute("href", 2); },
  ...
  if (support.getIdNotName) {
    ...
    Expr.filter.ID = function(id) {
      ...
      return function(elem) {
        const node = typeof elem.getAttributeNode !== strundefined &&
  ...
  contains =
      isNative(docElem.contains) || docElem.compareDocumentPosition ? function(a, b) {
...
Sizzle.matchesSelector = function(elem, expr) {
  ...
  if (support.matchesSelector && !documentIsXML &&
      (!rbuggyMatches || !rbuggyMatches.test(expr)) && !rbuggyQSA.test(expr)) {
    try {
...
Sizzle.error = function(
    msg) { throw new Error("Syntax error, unrecognized expression: " + msg); };
...
function siblingCheck(a, b) {
  let cur = b && a;
  const diff = cur && (~b.sourceIndex || MAX_NEGATIVE) -
...
Expr = Sizzle.selectors = {
  ...
  pseudos : {
    ...
    empty : function(elem) {
      ...
      //   Greater than "@" means alpha characters (specifically not starting
      //   with "#" or "?")
      for (elem = elem.firstChild; elem; elem = elem.nextSibling) {
    ...
    text : function(elem) {
      ...
      // etc) use getAttribute instead to test this case
    ...
    last : createPositionalPseudo(function(matchIndexes,
                                           length) { return [ length - 1 ]; }),

    eq : createPositionalPseudo(function(
        matchIndexes, length,
        argument) { return [ argument < 0 ? argument + length : argument ]; }),
...
function setMatcher(preFilter, selector, matcher, postFilter, postFinder,
  ...
  return markFunction(function(seed, results, context, xml) {
    ...
    const elems =
        seed || multipleContexts(selector || "*",
    ...
    let matcherOut =
        matcher ? // If we have a postFinder, or filtered seed, or non-seed
...
function matcherFromTokens(tokens) {
  ...
  const matchAnyContext =
      addCombinator(function(
                        elem) { return indexOf.call(checkContext, elem) > -1; },
                    implicitRelative, true);
  let matchers = [
    function(elem, context, xml) {
    ...
    if (match.length === 1) {
      ...
      while (i--) {
        ...
        if ((find = Expr.find[type])) {
          ...
          if ((seed =
                   find(token.matches[0].replace(runescape, funescape),
    ...
    return (
        ...
        (typeof selector === "string"
             ? // If this is a positional/relative selector, check membership in
               // the returned set so $("p:first").is("p:last") won't return
...
jQuery.each({
  ...
  parentsUntil : function(
      elem, i, until) { return jQuery.dir(elem, "parentNode", until); },
  ...
  nextUntil : function(
      elem, i, until) { return jQuery.dir(elem, "nextSibling", until); },
  prevUntil : function(
      elem, i, until) { return jQuery.dir(elem, "previousSibling", until); },
...
function winnow(elements, qualifier, keep) {
  ...
  if (jQuery.isFunction(qualifier)) {
    ...
    return jQuery.grep(
        elements, function(elem) { return (elem === qualifier) === keep; });
  } else if (typeof qualifier === "string") {
    const filtered =
        jQuery.grep(elements, function(elem) { return elem.nodeType === 1; });
  ...
  return jQuery.grep(
      elements,
      function(elem) { return jQuery.inArray(elem, qualifier) >= 0 === keep; });
}
...
jQuery.fn.extend({
  ...
  replaceWith : function(value) {
    ...
    // inserted this can help fix replacing a parent with child elements
...
function fixCloneNodeIssues(src, dest) {
  ...
  if (!jQuery.support.noCloneEvent && dest[jQuery.expando]) {
    ...
    // Event data gets referenced instead of copied if the expando gets copied
    // too
...
function getAll(context, tag) {
  ...
  let elem;
  let i = 0;
  let found = typeof context.getElementsByTagName !== core_strundefined
...
if (window.getComputedStyle) {
  ...
  curCSS = function(elem, name, _computed) {
    ...
    let ret = computed ? computed.getPropertyValue(name) || computed[name]
                       : undefined;
    const style = elem.style;
    ...
    if (computed) {
      ...
      // value" for margin-right Safari 5.1.7 (at least) returns percentage for
      // a larger set of values, but width seems to be reliably pixels this is
      // against the CSSOM draft spec:
  ...
  curCSS = function(elem, name, _computed) {
    ...
    // but not position css attributes, as those are proportional to the parent
    // element instead and we can't measure the parent instead because it might
...
function getWidthOrHeight(elem, name, extra) {
  ...
  if (val <= 0 || val == null) {
    ...
    // values for getComputedStyle silently falls back to the reliable
    // elem.style
  ...
  return (val +
          augmentWidthOrHeight(elem, name,
...
if (!jQuery.support.opacity) {
  jQuery.cssHooks.opacity = {
    ...
    set : function(elem, value) {
      ...
      // if setting opacity to 1, and no other filters exist - attempt to remove
      // filter attribute #6652 if value === "", then remove inline opacity
      // #12685
      if ((value >= 1 || value === "") &&
          jQuery.trim(filter.replace(ralpha, "")) === "" &&
          style.removeAttribute) {
        // Setting style.filter to null, "" & " " still leave "filter:" in the
        // cssText if "filter:" is present at all, clearType is disabled, we
        // want to avoid this style.removeAttribute is IE Only, but so
  ...
  if (!jQuery.support.reliableMarginRight) {
    jQuery.cssHooks.marginRight = {
      get : function(elem, computed) {
        if (computed) {
          // WebKit Bug 13343 - getComputedStyle returns wrong value for
          // margin-right Work around by temporarily setting element display to
          // inline-block
...
if (jQuery.expr && jQuery.expr.filters) {
  ...
  jQuery.expr.filters.visible = function(
      elem) { return !jQuery.expr.filters.hidden(elem); };
}
  ...
  serializeArray : function() {
    return this
        ...
        .map(function(i, elem) {
          ...
          return val == null
                     ? null
                     : jQuery.isArray(val) ? jQuery.map(val, function(val) {
                         return {
                           name : elem.name,
  ...
  if (self.length > 0) {
    jQuery
        ...
        .done(function(responseText) {
          ...
          self.html(selector ? // If a selector was specified, locate the right
  ...
  ajax : function(url, options) {
    ...
    if (!transport) {
      ...
      if (s.async && s.timeout > 0) {
        timeoutTimer =
            setTimeout(function() { jqXHR.abort("timeout"); }, s.timeout);
      }
  ...
  if (s.crossDomain) {
    ...
    return {
      send : function(_, callback) {
        ...
        // prepending Use native DOM manipulation to avoid our domManip AJAX
        // trickery
...
// unload
const xhrOnUnloadAbort = window.ActiveXObject && function() {
    ...
    if (!s.crossDomain || jQuery.support.cors) {
      ...
      return {
        send : function(headers, complete) {
          ...
          // ajaxSetup) For same-domain requests, won't change header if already
          // provided.
    ...
    function(prop, value) {
      ...
      if (parts) {
        ...
        if (unit !== "px" && start) {
          ...
          // if it uses the same units Fallback to end or a simple constant
        ...
        jQuery.fn[funcName] = function(margin, value) {
          ...
          return jQuery.access(this, function(elem, type, value) {
            ...
            if (elem.nodeType === 9) {
              ...
              // client[Width/Height], whichever is greatest unfortunately, this
              // causes bug #3838 in IE6/8 only, but there is currently no good,
              // small way to fix it.
