...
GZ3D.Manipulator = function (camera, mobile, domElement, doc) {
  ...
  let hovered = null;
  const hoveredColor = new THREE.Color();
  ...
  this.update = function () {
    ...
    const scale = (worldPosition.distanceTo(camPosition) / 6) * this.scale;
    this.gizmo.position.copy(worldPosition);
    this.gizmo.scale.set(scale, scale, scale);

    for (const i in this.gizmo.children) {
      for (const j in this.gizmo.children[i].children) {
        const object = this.gizmo.children[i].children[j];
  ...
  this.setMode = function (value) {
    ...
    for (const i in displayAxes[this.mode].children) {
  ...
  function onTouchStart(event) {
    ...
    const intersect = intersectObjects(event, pickerAxes[scope.mode].children);
    ...
    if (intersect) {
      ...
      const planeIntersect = intersectObjects(event, [
  ...
  function onMouseHover(event) {
    ...
    if (event.button === 0 && scope.selected === "null") {
      ...
      if (intersect) {
        ...
        // revert display axis color
        hoveredDisplay = displayAxes[scope.mode].getObjectByName(hovered.name);
        if (hoveredDisplay) {
          hoveredDisplay.material.color.copy(hoveredColor);
        }
