...
GZ3D.Scene.prototype.init = function () {
  ...
  // XYZ
  const XYZaxes = new THREE.Object3D();
  ...
  const mainAxis = new THREE.Object3D();
  ...
  const transAxis = new THREE.Object3D();
  ...
  const screwAxis = new THREE.Object3D();
  ...
  const ballVisual = new THREE.Object3D();
...
GZ3D.Scene.prototype.getRayCastModel = function (pos, intersect) {
  ...
  const objects = ray.intersectObjects(allObjects);

  let model;
  let point;
  if (objects.length > 0) {
    modelsloop: for (let i = 0; i < objects.length; ++i) {
...
GZ3D.Scene.prototype.createPointLight = function (
  ...
  const lightObj = new THREE.PointLight(color, intensity);
...
GZ3D.Scene.prototype.createSpotLight = function (
  ...
  const lightObj = new THREE.SpotLight(color, intensity);
  lightObj.distance = distance;
  ...
  const helperGeometry = new THREE.CylinderGeometry(0, 0.3, 0.2, 4, 1, true);
  helperGeometry.applyMatrix(new THREE.Matrix4().makeRotationX(Math.PI / 2));
...
GZ3D.Scene.prototype.createDirectionalLight = function (
  ...
  const lightObj = new THREE.DirectionalLight(color, intensity);
...
GZ3D.Scene.prototype.createRoads = function (points, width, texture) {
  ...
  geometry.dynamic = true;
  let texCoord = 0.0;
  const texMaxLen = width;
  let factor = 1.0;
  let curLen = 0.0;
  const tangent = new THREE.Vector3(0, 0, 0);
  let pA;
  let pB;
  const prevPt = new THREE.Vector3(0, 0, 0);
  let prevTexCoord;
  const texCoords = [];
  let j = 0;
...
GZ3D.Scene.prototype.loadHeightmap = function (
  ...
  // Sub-sample
  const col = (segmentWidth - 1) * scale;
  const row = (segmentHeight - 1) * scale;
  for (let r = 0; r < row; ++r) {
    for (let c = 0; c < col; ++c) {
      const index = (r * col * 1) / (scale * scale) + c * (1 / scale);
 ...
 * // loading using URI
 * // callback(mesh)
 * loadMeshFromUri('assets/house_1/meshes/house_1.dae', undefined, undefined,
 function(mesh)
            {
              // use the mesh
            });
 ...
 * // callback(mesh)
 * loadMeshFromString('assets/house_1/meshes/house_1.dae', undefined, undefined,
 function(mesh)
            {
              // use the mesh
            }, ['<?xml version="1.0" encoding="utf-8"?>
    <COLLADA xmlns="http://www.collada.org/2005/11/COLLADASchema"
 version="1.4.1"> <asset> <contributor> <author>Cole</author>
      ...
      if (
        ...
        if (centerSubmesh) {
          ...
          if (allChildren[i].geometry instanceof THREE.BufferGeometry) {
            const geomPosition = allChildren[i].geometry.attributes.position;
          ...
          // dae
            ...
            const vertices = allChildren[i].geometry.vertices;
...
GZ3D.Scene.prototype.showRadialMenu = function (e) {
  ...
  const pos = new THREE.Vector2(pointer.clientX, pointer.clientY);
...
GZ3D.Scene.prototype.showBoundingBox = function (model) {
  ...
  const position = this.boundingBox.geometry.attributes.position;
...
GZ3D.Scene.prototype.setViewAs = function (model, viewAs) {
  ...
  const showWireframe = viewAs === "wireframe";
  ...
  for (let i = 0; i < descendants.length; ++i) {
    if (
      ...
      if (descendants[i].material instanceof THREE.MultiMaterial) {
        ...
        for (let k = 0; k < descendants[i].material.length; ++k) {
...
GZ3D.Scene.prototype.viewJoints = function (model) {
  ...
  if (model.jointVisuals) {
    ...
    // Show: attach to parent
      ...
      for (let s = 0; s < model.joint.length; ++s) {
  ...
  // Create visuals
    ...
    model.jointVisuals = [];
    for (let j = 0; j < model.joint.length; ++j) {
      ...
      const jointVisual = this.jointAxis.XYZaxes.clone();
...
GZ3D.Scene.prototype.viewCOM = function (model) {
  ...
  if (model.COMVisuals) {
    ...
    // Show: attach to parent
      ...
      for (let s = 0; s < model.children.length; ++s) {
  ...
  // Create visuals
    ...
    let line_2;
    let line_3;
    let helperGeometry_1;
    let helperGeometry_2;
    let helperGeometry_3;
    let helperMaterial;
    ...
    for (let j = 0; j < model.children.length; ++j) {
      ...
      if (child.userData.inertial) {
        ...
        var userdatapose;
        let inertialPose = {};
        const inertial = child.userData.inertial;
...
GZ3D.Scene.prototype.updateLight = function (entity, msg) {
  ...
  if (msg.direction) {
    dir = new THREE.Vector3(msg.direction.x, msg.direction.y, msg.direction.z);
...
GZ3D.Scene.prototype.createFromSdf = function (sdf) {
  ...
  const sdfXml = this.spawnModel.sdfParser.parseXML(sdf);
  // sdfXML is always undefined, the XML parser doesn't work while testing
  // while it does work during normal usage.
  const myjson = xml2json(sdfXml, "\t");
  const sdfObj = JSON.parse(myjson).sdf;

  const mesh = this.spawnModel.sdfParser.spawnFromSDF(sdf);
  if (!mesh) {
    return;
  }
