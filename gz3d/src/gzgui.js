...
gzangular.controller("treeControl", [
  ...
  function ($scope) {
    ...
    $scope.expandProperty = function (
      ...
      let idContentOthers, idHeaderOthers;
...
GZ3D.Gui = function (scene) {
  ...
  this.emitter.on("openTab", function (id, parentId) {
    ...
    if (id.indexOf("propertyPanel-") >= 0) {
      const entityName = id.substring(id.indexOf("-") + 1);
      const object = that.scene.getByName(convertNameId(entityName, true));
  ...
  this.emitter.on("setPose", function (prop1, prop2, name, value) {
    ...
    const entity = that.scene.getByName(name);
...
GZ3D.Gui.prototype.setModelStats = function (stats, action) {
  ...
  if (action === "update") {
    ...
    // New model
    if (model.length === 0) {
      const thumbnail = this.findModelThumbnail(modelName);
...
GZ3D.Gui.prototype.formatStats = function (stats) {
  ...
  if (stats.inertial) {
    ...
    const inertialPose = stats.inertial.pose;
    inertial.pose = {};
...
var formatTime = function (time) {
  ...
  const timeDay = Math.floor(timeSec / 86400);
  timeSec -= timeDay * 86400;

  const timeHour = Math.floor(timeSec / 3600);
  timeSec -= timeHour * 3600;

  const timeMin = Math.floor(timeSec / 60);
  timeSec -= timeMin * 60;

  const timeMsec = Math.floor(timeNSec * 1e-6);
