...
GZ3D.RadialMenu.prototype.update = function() {
  ...
  for (let i = 0; i < this.numberOfItems; i++) {
    ...
    const newX = xdelta + this.startPosition.x;
    const newY = ydelta + this.startPosition.y;
...
GZ3D.RadialMenu.prototype.getPointer = function(event) {
  ...
  let posX = pointer.clientX - rect.left;
  let posY = pointer.clientY - rect.top;
...
GZ3D.RadialMenu.prototype.onLongPressMove = function(event) {
  ...
  let Selected = region - 4 + this.offset;
