describe("Sdf Parser tests", function () {
  ...
  describe("Spawn a world from SDF", function () {
    it("Should create a world THREE.Object3D", function () {
      ...
      let rot = obj3D.rotation.reorder("ZYX");
      ...
      // verify link
      const linkObj3D = obj3D.children[0];
      expect(linkObj3D.name).toEqual("link");

      // verify visual
      const visualObj3D = linkObj3D.children[0];
      ...
      // verify collision
      const colObj3D = linkObj3D.children[1];
      ...
      // verify nested link
      const nestedLinkObj3D = nestedObj3D.children[0];
      expect(nestedLinkObj3D.name).toEqual("nested_link");

      // verify visual
      const nestedVisualObj3D = nestedLinkObj3D.children[0];
      ...
      // verify collision
      const nestedColObj3D = nestedLinkObj3D.children[1];
      ...
      expect(rot.y).toBeCloseTo(lightRotation.y, 3);
      expect(rot.z).toBeCloseTo(lightRotation.z, 3);
      const lightObj = obj3D.children[0];
  ...
  describe("Add a model to the scene using custom urls", function () {
    it("should add a model to the scene and then remove it", function () {
      ...
      // Load SDF
      const obj = sdfparser.loadSDF(utilsPath + "house_2/model.sdf");
